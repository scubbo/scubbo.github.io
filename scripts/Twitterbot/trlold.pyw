import twitter, random, threading, time
...
class twitterBot:
  ...
  def update(self):
    ...
    f=file('lyrics.txt', 'r+')
    self.splut=[i[:-1] for i in f.readlines()]
    if self.splut[0] == '':
      self.used=[]
    else:
      self.used=[int(i) for i in self.splut[0].split(',')]
    if self.splut[-1]=='': self.splut=self.splut[:-1]
    self.lyrics=self.splut[1:]
  ...
  def post(self, tweet_number):
    try:
      self.api.PostUpdate(self.lyrics[tweet_number] + ' PLEASE RT')
      #print lyrics[tweet_number]
      self.used+=[tweet_number]
      f=file('lyrics.txt', 'w')
      f.writelines([str(i)+',' for i in self.used])
      f.seek(-1, 2)
      f.write('\n')
      f.writelines([i+'\n' for i in self.lyrics])
  ...
  def reply(self, recip):
    l=self.lyrics[random.choice([i for i in range(self.total) if i not in self.used])]
  ...
  #  smtpserver.ehlo()
  #  smtpserver.login('trexlyrics@gmail.com', 'Haiguyswhatsup')
  #  smtpserver.sendmail('trexlyrics@gmail.com', 'scubbojj@gmail.com', 'To: scubbojj@gmail.com\r\nFrom:   trexlyrics@gmail.com\r\nSubject: Trexlyrics has closed!\r\nAvenge me, father.')
  #  smtpserver.close()
  #
  #  import os
  #  os.chdir('D:\\Jack\'s Twitterbot')
  #  oo=file('oo.txt', 'w')
  #  oo.write('I crashed')
  #  oo.close()
  #
